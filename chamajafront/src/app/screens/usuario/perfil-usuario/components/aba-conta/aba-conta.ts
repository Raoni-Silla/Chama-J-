...
@Component({
  ...
  imports: [CommonModule, ProgressSpinnerModule, ToastModule, InputGroupModule, InputGroupAddonModule, FormsModule, FileUploadModule, ButtonModule, ReactiveFormsModule],
...
export class AbaConta implements OnInit {
  ...
  atualizarSenha() {
    if (this.senhaForm.valid) {
      ...
      this.usuarioService.trocarSenha(this.senhaForm.get('senhaAtual')?.value ?? '', this.senhaForm.get('novaSenha')?.value ?? '').subscribe({
        next: () => {
  ...
  get temTamanho() { return this.valorSenha.length >= 8; }
  get temMaiuscula() { return /[A-Z]/.test(this.valorSenha); }
  get temMinuscula() { return /[a-z]/.test(this.valorSenha); }
  get temNumero() { return /\d/.test(this.valorSenha); }
  get temEspecial() { return /[@$!%*?&#_-]/.test(this.valorSenha); }

  get senhasBatem() {
    return (this.senhaForm.get('novaSenha')?.value ?? '') === (this.senhaForm.get('confirmarSenha')?.value ?? '');
  }
